...
import { EVENT_TYPE, SyntaxTree, History, HistoryEntry, MachineStateValue, STByRangeRequest, SyntaxTreeResponse, UndoRedoManager, VisualizerLocation, webviewReady, MACHINE_VIEW, DIRECTORY_MAP, SCOPE, ProjectStructureResponse, ArtifactData, ProjectStructureArtifactResponse, CodeData, getVisualizerLocation } from "@wso2/ballerina-core";
...
const stateMachine = createMachine<MachineContext>(
        ...
        states: {
            ...
            renderInitialView: {
                invoke: {
                    ...
                    //     {
                    //         target: "activateLS",
                    //         cond: (context, event) => !context.langClient,
                    //     },
                    //     {
    ...
    services: {
        ...
        reloadProject: (context, event) => {
            // Get context values from the project storage so that we can restore the earlier state when user reopens vscode
            return new Promise<void>((resolve, reject) => {
                // updateView(true, context.projectUri);
                resolve();
            });
        }
    }
...
//                 actions: [
//                     assign({
//                         isBI: (_context, event) => event.payload.isBI,
//                         projectUri: (_context, event) => event.payload.projectPath,
//                         scope: (_context, event) => event.payload.scope,
//                         org: (_context, event) => event.payload.orgName,
//                         package: (_context, event) => event.payload.packageName,
//                     }),
//                     async (context) => {
//                         await new Promise(resolve => setTimeout(resolve, 5000));
//                         updateView(true, context.projectUri);
...
//     sendEvent: (eventType: EVENT_TYPE) => { stateService.send({ type: eventType }); },
//     updateProjectStructure: (payload: ProjectStructureResponse) => { stateService.send({ type: "UPDATE_PROJECT_STRUCTURE", payload }); },
//     switchProject: (payload: { isBI: boolean, projectPath: string, scope?: SCOPE, orgName?: string, packageName?: string }) => { stateService.send({ type: "SWITCH_PROJECT", payload }); },
//     resetToExtensionReady: () => {
//         stateService.send({ type: 'RESET_TO_EXTENSION_READY' });
//     },
// };
...
//     const payload = await checkForProjects();
//     StateMachine.switchProject(payload);
// }
