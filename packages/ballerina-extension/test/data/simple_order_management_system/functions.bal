...
public function getOrderById(string orderId) returns Order|OrderNotFoundError|error {
    sql:ParameterizedQuery query = `SELECT orderId, customerId, status, createdAt, totalAmount, currency, shippingAddress, billingAddress, orderLines FROM orders WHERE orderId = ${orderId};`;
...
function publishOrderEvent(OrderCreatedEvent eventPayload) returns error? {
...
function calculateTotal(OrderLinePayload[] lines) returns decimal {
    ...
    foreach var line in lines {
        total += <decimal>99.99 * line.quantity; // Use mocked price
    }
    return total;
}
