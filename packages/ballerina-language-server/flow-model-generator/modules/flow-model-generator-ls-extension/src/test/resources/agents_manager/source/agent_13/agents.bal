...
ai:AzureOpenAiProvider myModel = check new ({auth: {apiKey}}, serviceUrl, deploymentId, apiVersion);
...
final ai:Agent agent = check new (model = myModel,
    ...
    tools = [mcpServer], agentType = ai:REACT_AGENT, memory = new ai:MessageWindowChatMemory(20));
...
final ai:McpToolKit mcpServer = check new ("http://localhost:8000", permittedTools = (), info = {name: "MCP Server", version: ""})