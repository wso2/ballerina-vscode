...
service OASServiceType on new http:Listener(9090) {
	...
	resource function get pet() returns int|http:NotFound {
        do {
            ...
            Store[] stores = from var bank in banks select {name: bank.name, credentials: {username: bank.userInfo}};
            StoreList storeList = {stores: from var bank in banks select {name: bank.name, credentials: {username: bank.userInfo}}};
		} on fail error e {
