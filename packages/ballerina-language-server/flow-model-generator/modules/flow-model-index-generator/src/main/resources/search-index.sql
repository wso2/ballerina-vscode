...
CREATE VIRTUAL TABLE PackageFTS USING fts5
(
    package_name,
    description,
    keywords,
...
INSERT INTO PackageFTS(PackageFTS, rank)
VALUES ('rank', 'bm25(10.0, 2.0, 5.0)');
INSERT INTO ConnectorFTS(ConnectorFTS, rank)
VALUES ('rank', 'bm25(10.0, 2.0, 5.0)');
INSERT INTO FunctionFTS(FunctionFTS, rank)
VALUES ('rank', 'bm25(10.0, 2.0, 5.0)');
INSERT INTO TypeFTS(TypeFTS, rank)
VALUES ('rank', 'bm25(10.0, 2.0, 5.0)');

-- Create triggers to keep FTS tables in sync with main tables
CREATE TRIGGER package_ai
    AFTER INSERT
    ON Package
BEGIN
    INSERT INTO PackageFTS(rowid, package_name, description, keywords)
    VALUES (new.id, new.package_name, new.description, new.keywords);
END;

CREATE TRIGGER package_ad
    AFTER DELETE
    ON Package
BEGIN
    INSERT INTO PackageFTS(PackageFTS, rowid, package_name, description, keywords)
    VALUES ('delete', old.id, old.package_name, old.description, old.keywords);
END;

CREATE TRIGGER package_au
    AFTER UPDATE
    ON Package
BEGIN
    INSERT INTO PackageFTS(PackageFTS, rowid, package_name, description, keywords)
