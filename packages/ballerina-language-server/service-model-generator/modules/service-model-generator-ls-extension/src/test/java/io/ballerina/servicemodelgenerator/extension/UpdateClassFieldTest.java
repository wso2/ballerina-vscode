...
/**
 * Tests for the service model source generator updateClassField service.
 *
 * @since 1.0.0
 */
public class UpdateClassFieldTest extends AbstractLSTest {
    ...
    public void test(Path config) throws IOException {
        ...
        ClassFieldModifierRequest request = new ClassFieldModifierRequest(
                sourceDir.resolve(testConfig.filePath()).toAbsolutePath().toString(), testConfig.field());
        ...
        if (assertFailure) {
            TestConfig updatedConfig =
                    new TestConfig(testConfig.filePath(), testConfig.description(), testConfig.field(), newMap);
//            updateConfig(configJsonPath, updatedConfig);
    ...
    protected String getResourceDir() {
        return "update_class_field";
    }

    @Override
    protected Class<? extends AbstractLSTest> clazz() {
        return UpdateClassFieldTest.class;
    }
