...
public class ReadOnlyMetadataManager {
    ...
    public Map<String, List<String>> extractForKind(String kind, ServiceDeclarationNode serviceNode,
        ...
        List<ReadOnlyMetaData> metadataList = getMetadataFromDatabase(context);
        List<ReadOnlyMetaData> filteredMetadata = metadataList.stream()
    ...
    /**
     * Checks if an extractor is registered for the given kind.
     *
     * @param kind The metadata kind to check
     * @return true if an extractor is available, false otherwise
     */
    public boolean hasExtractorForKind(String kind) {
        return extractors.containsKey(kind) || "CUSTOM".equals(kind);
    }

    /**
     * Gets the available extractor kinds.
     *
     * @return Set of supported kinds
     */
    public java.util.Set<String> getSupportedKinds() {
        java.util.Set<String> kinds = new java.util.HashSet<>(extractors.keySet());
        kinds.add("CUSTOM");
        return kinds;
    }
}