...
/**
 * Code lens provider for main function debug.
 *
 * @since 1.0.1
 */
@JavaSPIService("org.ballerinalang.langserver.commons.codelenses.spi.LSCodeLensesProvider")
public class MainDebugCodeLensProvider extends AbstractCodeLensesProvider {

    public MainDebugCodeLensProvider() {
        super("main.debug.CodeLenses");
    }
    ...
    public boolean validate(Node node) {
        if (node instanceof FunctionDefinitionNode functionDefinitionNode) {
            return "main".equals(functionDefinitionNode.functionName().text());
        }
        return false;
    }
    ...
    public CodeLens getLens(DocumentServiceContext context, Node node) {
        ...
        Command command = new Command("Debug", "ballerina.source.debug", args);
