...
public record ImportMuleResponse(String error, Map<String, String> textEdits, String report) {

    public static ImportMuleResponse from(ToolExecutionResult result) {
        return new ImportMuleResponse(result.error(), result.textEdits(), result.report());
    }
}