...
export const getAgentFilePath = async (rpcClient: BallerinaRpcClient) => {
    ...
    const agentFilePath = await rpcClient.getVisualizerRpcClient().joinProjectPath({ segments: ['agents.bal'] });
...
export const removeAgentNode = async (agentCallNode: FlowNode, rpcClient: BallerinaRpcClient): Promise<boolean> => {
    ...
    const agentFilePath = await rpcClient.getVisualizerRpcClient().joinProjectPath({ segments: [agentFileName] });
    ...
    const modelFilePath = await rpcClient.getVisualizerRpcClient().joinProjectPath({ segments: [modelFileName] });
