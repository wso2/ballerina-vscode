...
export function FTPForm(props: FTPFormProps) {
    ...
    // Filter non-enabled functions for dropdown options
    const nonEnabledFunctions = serviceModel.functions?.filter(fn => !fn.enabled && fn.metadata?.label==="onCreate") || [];

    // Reset form state when model prop changes
    useEffect(() => {
        setserviceModel(model);
        // Set initial function model based on first non-enabled function
    ...
    const canDataBind = functionModel?.properties?.canDataBind;
        ...
        <>
            ...
            <SidePanelBody>
                <EditorContentColumn>
                    ...
                    {fileInfoParameter || callerParameter ? <Divider /> : null}
